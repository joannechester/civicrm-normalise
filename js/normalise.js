cj(function($) {
  ...
  function normalise (dom) {
    ...
    if (v == v.toLowerCase() || v == v.toUpperCase()) {
      $(dom).val(v[0].toUpperCase() + v.slice(1).toLowerCase());
      }
  ...
  function upperfirst (jad) {
    var v=$(jad).val();
    if (v == v.toLowerCase() || v == v.toUpperCase()) {
      $(jad).val(v.split(" ").map(function(i){return i[0].toUpperCase() + i.substring(1).toLowerCase()}).join(" ").replace(/(\d+)([a-z]+)/, function(a) { return a.toUpperCase() 
      }));
    }
  }
